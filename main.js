...
function generarEntrada(IDEvento,dni,sector,cantidad,usuario,ruta){
  ...
  if(capa.innerHTML == 'Generar entrada' || capa.innerHTML == 'Generar entradas'){
  capa.innerHTML="<img src='"+ruta+"/images/ajax.gif' style='width:24px;vertical-align:middle;'>";
  var ajax=valorar();
    ajax.open("POST", ruta+"/agregarEntrada.php", true);
    ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    ajax.send("IDEvento="+IDEvento+"&dni="+dni+"&sector="+sector+"&cantidad="+cantidad+"&usuario="+usuario);
...
function actualizarTotal(IDEvento,cantidad,sector,ruta){
  ...
  if(document.getElementById("botonGenerarEntrada"+IDEvento).innerHTML == 'Generar entrada' || document.getElementById("botonGenerarEntrada"+IDEvento).innerHTML == 'Generar entradas' ){
 if(cantidad > 1)
    document.getElementById("botonGenerarEntrada"+IDEvento).innerHTML="Generar entradas";
  else
  document.getElementById("botonGenerarEntrada"+IDEvento).innerHTML="Generar entrada";
  }
  var ajax=valorar();
    ajax.open("POST", ruta+"/actualizarTotal.php", true);
    ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    ajax.send("IDEvento="+IDEvento+"&cantidad="+cantidad+"&sector="+sector);
    ajax.onreadystatechange=function()
   {
      if (ajax.readyState==4)
      {
